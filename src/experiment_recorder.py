...
class ExperimentRecorder:
    ...
    def record_individual_evaluation( # TODO this should not include nn.individual.py stuff in this file, but we should create an inherited class of this in nn package which does
        ...
        # Save loss curve outside the lock (per-individual file, no contention on self.data)
        if loss_curve:
    ...
    def _save_loss_curve(self, individual_id: int, generation: int, loss_curve: list) -> str:
        ...
        # Each individual has a unique id so no cross-thread collision here,
        # but add thread id for safety.
        temp_filepath = filepath + f".tmp.{threading.get_ident()}"
    ...
    def _save(self) -> None:
        with self._lock:
            # Use thread-id in temp name to avoid collisions from concurrent callers
            temp_path = self.record_path + f".tmp.{threading.get_ident()}"
